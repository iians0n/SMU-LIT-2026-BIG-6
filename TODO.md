...
**Now:** M5 complete · **Owner on deck:** Anson · **Done:** Hour 0 → M5, all on `main`

---

**Scripts:** `npm run dev` · `build` · `typecheck` · `test` · `demo` (rehearse the §10 narrative) · `fixtures` (emit + verify against disk) · `check:fixtures` (run the 36-expectation oracle)
...
- [x] **FR04 case record + timeline UI** — reads the fixture, editable, versioned
...
- [x] **FR03 document intake** — upload, parse, OCR, hash + dedupe, excerpt→page/region anchors
...
- [x] **FR03 file states UI** — unreadable · password-protected · truncated · unsupported · possibly unrelated, plus retry/replace/retain/remove
      *Done when:* an unsupported type is visibly unsupported and never looks read
- [x] **FR02 question planner** — unresolved-info list, one question at a time, why-it-matters, I don't know / Skip
...
- [x] **FR01 voice + text intake** — Web Speech API, start/pause/stop/review, pre-activation notice
...
- [x] **FR05 evidence matrix** — issue · facts · supporting · conflicting · status · reason · next question
...
- [x] **FR10 contradiction detector** — inconsistent dates/amounts, partial performance, changed terms, refunds, settlement attempts, adverse docs
      *Done when:* each finding cites its excerpt; two plausible readings surface both plus the distinguishing fact
- [x] **`GET /api/case/contradictions`**
...
- [x] Staleness end-to-end: edit a fact → Clarence's draft flips to Needs review
- [x] Verification events flushing from Anson's pipeline to Clarence's store

## M5 — Evaluation (both)

- [x] Scenario 2 missing/poor evidence · 3 contradiction · 7a mic + upload recovery (Anson)
- [x] Scenario 4 rule boundaries · 6 grounding · 7b export/keyboard/access (Clarence)
- [x] Scenario 1 full run · 5 change propagation (joint)
- [x] Demo rehearsed twice, including the unsupported-case referral
