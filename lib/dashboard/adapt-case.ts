...
export function adaptCaseRecord(record:CaseRecord,ownerId=record.case.ownerId):Case{
 ...
 const documentedKinds=new Set(record.facts.filter(f=>f.origin==='document_extracted'&&f.excerptIds.length>0).map(f=>f.kind));
 const effectiveFacts=record.facts.filter(f=>!documentedKinds.has(f.kind)||f.origin==='document_extracted');
 const facts:Fact[]=record.facts.map((fact,index)=>({id:fact.id,key:documentedKinds.has(fact.kind)&&fact.origin!=='document_extracted'?`superseded_${fact.kind}_${index}`:factKey(fact,index),label:fact.statement,value:fact.amount?.minorUnits??fact.date?.value??fact.statement,origin:fact.origin,confirmedByUser:fact.confirmedByUser,disputed:fact.disputed,unknown:fact.unknown,sourceLinks:sourceRefs(fact),extractionConfidence:fact.excerptIds.length?Math.min(...fact.excerptIds.map(id=>record.excerpts.find(e=>e.id===id)?.extractionConfidence??0)):null}));
