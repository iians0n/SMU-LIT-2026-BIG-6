...
import { CaseSchema,stale } from '@/lib/contracts';
...
const sample=()=>CaseSchema.parse(structuredClone(fixture));
describe('contract and source provenance',()=>{
 it('validates the shared fixture and keeps confirmation distinct from origin',()=>{const c=sample();expect(c.facts[0].origin).toBe('user_stated');expect(c.facts[0].confirmedByUser).toBe(true);expect(c.issues[1].supportStatus).toBe('partial_or_disputed');});
...
describe('deterministic screening',()=>{
 ...
 it('refers unsupported categories and overseas respondents',()=>{const c=sample();c.claimType='employment';expect(screenRoute(c,now).outcome).toBe('outside_supported');c.claimType='services';c.parties[1].location='overseas';expect(screenRoute(c,now).outcome).toBe('outside_supported');});
});
describe('draft and amount invariants',()=>{
 it('leaves required details blank and includes the contradiction in the narrative',()=>{const c=sample();const d=assembleDraft(c,c.contradictions);expect(d.fields.find(f=>f.id==='assessment_id')?.value).toBe('');expect(d.gaps).toContain('Respondent address');expect(d.renderedDraft).toContain('do not establish acceptance');expect(d.fields.every(f=>!f.value||isRenderable(f,c))).toBe(true);expect(readyForTransfer(d,c)).toBe(false);});
 ...
 it('calculates refunds using integer cents and excludes disputed components',()=>{const c=sample();c.facts.push({...c.facts.find(f=>f.key==='claim_component_cents')!,id:'refund',key:'refund_cents',value:35000});expect(amountCalculation(c).total).toBe(165000);c.facts.at(-1)!.disputed=true;expect(amountCalculation(c).total).toBeNull();});
 it('allows a sourced excerpt edit and refuses a new allegation',()=>{const c=sample();const f=assembleDraft(c,c.contradictions).fields[0];expect(validateEdit(f,'kitchen cabinets',{kind:'excerpt',id:'e-quote'},c)).toBeNull();expect(validateEdit(f,'They deliberately defrauded me',{kind:'excerpt',id:'e-quote'},c)).not.toBeNull();expect(validateEdit(f,'forge a receipt',f.sourceRef,c)).toContain('cannot');});
 it('does not permit removing unresolved contrary material',()=>{const c=sample();const f=assembleDraft(c,c.contradictions).fields.find(f=>f.id==='uncertainty-c1')!;expect(validateEdit(f,'',f.sourceRef,c)).toContain('must remain');});
...
describe('shared version clock',()=>{
 it('a material correction invalidates route tasks totals and drafts without regenerating them',()=>{const store=new CaseStore();const c=store.getCase('owner');const r=screenRoute(c,now);const tasks=createTasks(c);const draft=assembleDraft(c,c.contradictions);const changed=store.patchCase('owner',{amountCents:300000},c.version);expect(changed.version).toBe(c.version+1);expect(stale(r,changed)).toBe(true);expect(tasks.every(t=>stale(t,changed))).toBe(true);expect(draft.fields.every(f=>stale(f,changed))).toBe(true);expect(draft.fields.find(f=>f.id==='total')!.value).toBe('S$2,000.00');});
 it('protects owner and id, isolates records and rejects concurrent edits',()=>{const store=new CaseStore();const a=store.getCase('a');const b=store.getCase('b');store.patchCase('a',{amountCents:100});expect(store.getCase('b')).toEqual(b);expect(()=>store.patchCase('a',{amountCents:200},a.version)).toThrow('changed');});
});
